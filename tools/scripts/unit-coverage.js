...
const testListArm = args.pop();

let fileContent, fileContentArm;
let root = false;
...
if (safeExistsSync(testListArm)) {
    fileContentArm = safeReadFileSync(testListArm).toString();
} else {
    fileContentArm = safeReadFileSync('./test/' + testListArm).toString();
    root = true;
}
